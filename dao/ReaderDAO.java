...
public class ReaderDAO {
    ...
    /**
     * HÀM MỚI: Thêm một độc giả mới.
     * Lưu ý: Cần xử lý logic tạo tài khoản (Users) tương ứng ở Controller.
     */
    public void addReader(Reader reader) {
        String sql = "INSERT INTO Readers (maDocGia, hoTen, diaChi, soDienThoai, userId) " +
                     "VALUES (?, ?, ?, ?, ?)";
        try (Connection conn = DBConnect.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public void updateReader(Reader reader) {
        String sql = "UPDATE Readers SET maDocGia = ?, hoTen = ?, diaChi = ?, soDienThoai = ? " +
                     "WHERE readerId = ?";
        try (Connection conn = DBConnect.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    /**
     * HÀM MỚI: Xóa một độc giả.
     * Lưu ý: Cần xử lý xóa (hoặc vô hiệu hóa) tài khoản Users tương ứng.
     * Và kiểm tra xem độc giả có đang mượn sách không trước khi xóa.
     */
