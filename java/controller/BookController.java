...
public class BookController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		switch (action) {
		case "new":
			showForm(request, response); // Chuyển sang trang thêm mới
			break;
		case "edit":
			editForm(request, response); // Chuyển sang trang sửa
			break;
		case "delete":
			deleteBook(request, response); // Thực hiện xóa
			break;
		default:
			listBooks(request, response); // Mặc định thì mới hiện danh sách
			break;
		}
	}
